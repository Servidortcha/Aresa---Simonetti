-- ============================================================
-- 0022_ajuste_encargado.sql
-- Permite al encargado (zeballes) ajustar el stock de sus cajas
-- de acopio (depósito Gral. Villegas) vía el control de stock.
-- El admin sigue pudiendo ajustar cualquier depósito.
--
...
begin
  ...
  if v_rol is null or v_rol not in ('admin', 'encargado') then
    return query select false, 'No tenés permiso para ajustar el stock.', null::numeric;
    return;
  end if;
  ...
  select stock, deposito, unidad, nombre into v_stock, v_deposito, v_unidad, v_nombre
  from public.insumos
  ...
  -- El encargado solo puede ajustar su depósito (Gral. Villegas)
  if v_rol = 'encargado' and v_deposito <> 'Gral. Villegas' then
    return query select false, 'Solo podés ajustar el stock de tus cajas de acopio.', null::numeric;
    return;
  end if;
